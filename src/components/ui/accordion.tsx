...
const AccordionTrigger = ({ className, children, ...props }: React.ComponentProps<typeof AccordionPrimitive.Trigger>) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      className={cn("flex flex-1 items-center justify-between py-4 font-medium text-black transition-all hover:opacity-70 [&[data-state=open]>svg]:rotate-180", className)}
      {...props}
    >
      {children}
