...
const AvatarsCircle = ({ avatars }: { avatars: string[] }) => {
  ...
  return (
    ...
    >
      <div className="absolute w-full h-full rounded-full border-2 border-border" />
      {avatars.map((avatar, index) => {
        ...
        return (
          <MotionImage
            ...
            animate={{ transform: 'translate(-50%, -50%) rotate(-360deg)', transition: { duration: 20, repeat: Infinity, ease: 'linear' } }}
            alt="Avatar"
            src={avatar}
            width={200}
            height={200}
...
export default function Spinner({ avatars, labels, className }: SpinnerProps) {
  return (
    <div className={cn('relative w-full h-full flex-1 items-center justify-center overflow-hidden', className)}>
